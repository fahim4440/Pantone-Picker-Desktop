...
class ColorPickerBloc extends Bloc<ColorPickerEvent, ColorPickerState> {
  ...
  ColorPickerBloc() : super(ColorPickerInitial()) {
    ...
    on<ColorPickedEvent>((event, emit) async {
      final img.Pixel extractedColor = await _colorRepository.onTapDown(event.image, event.position, event.photoViewController, event.scrollController, event.context);
